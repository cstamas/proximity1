...
public class DefaultExpiringProxyingLogic extends DefaultProxyingLogic {
	...
	public ProxiedItem afterLocalCopyFound(ProxiedItem item, Repository repository) {
		if (item.getProperties().getMetadata(DefaultExpiringProxyingLogic.METADATA_EXPIRES) != null) {
			...
			Date expires = new Date(Long.parseLong(item.getProperties().getMetadata(DefaultExpiringProxyingLogic.METADATA_EXPIRES)));
	...
	public ProxiedItem afterRemoteCopyFound(ProxiedItem item, Repository repository) {
		if (itemExpirationPeriod != -1) {
			...
			item.getProperties().setMetadata(DefaultExpiringProxyingLogic.METADATA_EXPIRES,
