...
public class WritableFileSystemStorage extends ReadOnlyFileSystemStorage {
	...
	logger.debug("Storing item in [{}] in storage directory {}", item.getProperties().getPath(), getStorageBaseDir());
	try {
		...
		if (isMetadataAware()) {
		    item.getProperties().putAllMetadata(
			    getProxiedItemPropertiesFactory().expandItemProperties(item.getProperties().getPath(), file, false).getAllMetadata());
	...
	while (!stack.isEmpty()) {
		...
		if (ip.isDirectory()) {
			...
			logger.debug("We have an " + DefaultExpiringProxyingRepositoryLogic.METADATA_EXPIRES + " property");
