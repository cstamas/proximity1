...
public class LogicDrivenRepositoryImpl extends AbstractRepository {
    ...
    protected Item doRetrieveItem(ProximityRequest request) throws RepositoryNotAvailableException, ItemNotFoundException, StorageException {
	...
	try {
	    ...
	    if (!isOffline() && !request.isLocalOnly() && getRepositoryLogic().shouldCheckForRemoteCopy(this, request, localResult)
		...
		result.getProperties().setRemoteUrl(getRemoteStorage().getAbsoluteUrl(result.getProperties().getPath()));
	    }
