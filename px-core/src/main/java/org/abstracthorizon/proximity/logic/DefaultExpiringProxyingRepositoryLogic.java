...
public class DefaultExpiringProxyingRepositoryLogic extends DefaultProxyingRepositoryLogic {
    ...
    /**
         * If item has defined EXPIRES metadata, will use it and remove item
         * from repository if needed.
         */
	...
	if (item.getProperties().getMetadata(DefaultExpiringProxyingRepositoryLogic.METADATA_EXPIRES) != null) {
	    ...
	    Date expires = new Date(Long.parseLong(item.getProperties().getMetadata(DefaultExpiringProxyingRepositoryLogic.METADATA_EXPIRES)));
    ...
    /**
         * If expiration period is not NO_EXPIRATION, it will apply it on all
         * items.
         */
	...
	if (itemExpirationPeriod != NO_EXPIRATION) {
	    ...
	    remoteItem.getProperties().setMetadata(DefaultExpiringProxyingRepositoryLogic.METADATA_EXPIRES, Long.toString(expires.getTime()));
	}
	return remoteItem;
    }
