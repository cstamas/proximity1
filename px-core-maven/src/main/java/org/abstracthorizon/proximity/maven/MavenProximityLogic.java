...
public class MavenProximityLogic extends DefaultProximityLogic {
	...
	if (listOfProxiedItems.size() > 1) {
		...
		File tmpFile = new File(System.getProperty("java.io.tmpdir"), request.getPath().replace(ItemProperties.PATH_SEPARATOR.charAt(0), '_'));
		if (tmpFile.exists()) {
		...
		for (int i = 0; i < listOfProxiedItems.size(); i++) {
			...
			logger.warn("Could not merge M2 metadata: " + currentItem.getProperties().getDirectoryPath() + " from repository "
				+ currentItem.getProperties().getRepositoryId(), ex);
		    } catch (IOException ex) {
			logger.warn("Got IOException during merge of M2 metadata: " + currentItem.getProperties().getDirectoryPath()
	...
	File tmpFile = new File(System.getProperty("java.io.tmpdir"), req.getPath().replace(ItemProperties.PATH_SEPARATOR.charAt(0), '_') + "."
